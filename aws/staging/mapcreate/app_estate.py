...
DATABASE_NAME = "TestScrapingResults"
DATAFRAME_DTYPE = {
    "price per unit area": float,
    "area": float,
    "age_years": int,
    "age_months": int,
    "price": int,
...
dataframe = dataframe.astype(DATAFRAME_DTYPE)
...
with st.sidebar:
    ...
    price_range = st.slider("areaの指定", min_area, max_area, (min_area, max_area), step=1.0)

    min_age_years = dataframe["age_years"].min()
    max_age_years = dataframe["age_years"].max()
    age_years_range = st.slider("age_yearsの指定", min_age_years, max_age_years, (min_age_years, max_age_years))
...
if "map" in selected["indices"]:
    ...
    st.session_state.candidates = pd.concat(
        [st.session_state.candidates, dataframe[dataframe["address"] == selected_address]]
    )
